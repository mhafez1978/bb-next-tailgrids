...
// Define a function to dynamically generate metadata
export async function generateMetadata({
  ...
  try {
    ...
    const res = await fetch(
      `https://api.blooming-brands.com/wp-json/wp/v2/posts/${params.id}?_embed`,
      { next: { revalidate: 60 } } // Optionally cache the request for 60 seconds
    );
    ...
    if (post) {
      title = `Blooming Brands | ${post.title.rendered}`;
      description =
        post.excerpt?.rendered?.replace(/(<([^>]+)>)/gi, "") || description; // Strip HTML tags from excerpt
    }
